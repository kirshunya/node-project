...
class dominoGameService {
  ...
  async startLobby(ws, aWss, msg, startTurn) {
    ...
    }, 5000); //Kamal oyunun bashlama vaxti 5-4-3-2-1-0
  }
  ...
  async checkAvailableTurn(
    ...
    if (market.length > 1 && !availableTurn) {
      // открыть базар у кого не хватает очков
      for (const client of aWss.clients) {
        if (
      ...
      // показать всем что у игрока открыт базар
      for (const client of aWss.clients) {
        if (
          client.dominoRoomId == roomId &&
          client.tableId == tableId &&
          ...
          client.send(
            JSON.stringify({ method: "enemyOpenMarket", userId: user.userId })
          );
        }
      }
    } else if (
  ...
  async endClassicDominoGame(
    ...
    setTimeout(async () => {
      ...
      // удаляем всех кто подключен к вебсокету
      ...
      // получаем пользователей с базы которые остались в этой игре
      let thisGamePlayers = await DominoGamePlayer.findAll({
        where: { dominoGameId: dominoGame.id },
      });

      // удляем их подключенные вебсокеты
      for (const client of aWss.clients) {
        for (const thisPlayer of thisGamePlayers) {
          console.log(client.userId + " " + thisPlayer.userId);
          if (client.userId == thisPlayer.userId) {
            if (
              client.dominoRoomId != null &&
              client.tableId != null &&
              client.playerMode != null &&
              ...
              client.send(
                JSON.stringify({
                  method: `NUM-${thisPlayer.userId}=${client.userId}`,
                })
              );
              client.dominoRoomId = null;
              client.tableId = null;
    ...
    }, 10000); //Kamal oyunun bitme vaxti 10-9- 0 ra qeder
  }
